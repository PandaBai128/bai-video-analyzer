...
export function buildVideoTimelinePrompt(input: {
  readonly metadata: VideoMetadata;
  readonly subtitles?: readonly SubtitleCue[];
  readonly outputLocale?: UiLocale;
}): string {
  // Round 23 必修 B1：时间线是导航，不是文章大纲
  // - 字幕行带 #N cue id 编号（从 0 开始）
  // - 模型的 chapter / segment 用 startCueId / endCueId 指向字幕行
  // - 后端按 cue id 映射回真实时间（防御模型把 0:39 标成 0:50 才开始的主题）
...
请只输出合法 JSON，不要 Markdown，不要解释。格式如下：
...
- 如果你同时输出 "timestamp" 和 "startCueId"，以 startCueId 为准，timestamp 会被忽略。
...
约束：
- 只输出示例中列出的字段（overview / chapters / startCueId / endCueId / importance / contentTag / title / summary / segments），不要输出 timestamp / endTimestamp / reasoning / quotes / keyConcepts / timeline / coreTakeaways / reviewSummary / inspirations / watchStrategy。
- **不要**再生成复盘总结（要点、整体反思、行动建议等）。复盘会在后续结合时间线、提问和你的标注单独生成。
- **不要**为美观而把章节凑成整分钟（247s 就是 247s，不要改 240s）。
- ${chapterDensityGuidance}
- 每章 1-3 个 segment。
- endCueId 必须 ≥ startCueId，不能反向。
- segment 的 cue range 必须完全落在父 chapter 的 cue range 范围内（startCueId >= chapter.startCueId 且 endCueId <= chapter.endCueId）。
...
function createOutputLanguageInstruction(locale: UiLocale): string {
  if (locale === 'en-US') {
    return [
      ...
      'Hard rule: every user-visible JSON string value such as overview, title, and summary must be English even when subtitles and this prompt contain Chinese.',
  ...
  return '默认用中文输出。所有用户可见 JSON 字符串使用中文，schema 字段名和枚举值保持英文。';
}
...
function createOutputExamples(locale: UiLocale): {
  ...
  return {
    ...
    chapterSummary: '1-2 句话说明这一段讲什么，方便用户决定是否跳转观看',
